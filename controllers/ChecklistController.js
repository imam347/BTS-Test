...
const ChecklistItem = db.checklistItem
    ...
    if (! name) {
      return res.status(400).json({
        status: 400,
        message: 'Please insert name for the checklist.',
      })
    }
    ...
    exports.deleteChecklist = async (req, res) => {
        try {
          ...
          const checklist = await Checklist.findOne({ where: { id: checklistID, user_id: userId } }); 
          ...
          const items = await ChecklistItem.findAll({ where: { id: checklistID } }); 
